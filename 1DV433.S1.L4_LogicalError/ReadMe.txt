...
3. Use the "cd" command (which stands for "change directory") to set the path 
	"1dv433(cpp)\step1\4_logicalerror" to the current folder 
	(or other location where you saved the folder structure).

4. Compile the source file using the command "cl -c -EHsc logicalerror.cpp"
...
6. Link the program with the command "link " + object filename (ie "logicalerror.obj")

7. Run the program: "logicalerror.exe"
